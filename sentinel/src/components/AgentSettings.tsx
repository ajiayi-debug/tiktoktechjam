...
type BindEvt = { detail?: { value?: string } };

export default function AgentSettings(props: {
  configs: AgentConfig[];
  onSave: (next: AgentConfig) => void;
}) {
  const toggle = useCallback((cfg: AgentConfig) => {
    // eslint-disable-next-line no-unused-expressions
    'background only';
    props.onSave({ ...cfg, enabled: !cfg.enabled });
  }, [props]);

  const clear = useCallback((cfg: AgentConfig) => {
    // eslint-disable-next-line no-unused-expressions
    'background only';
    props.onSave({ ...cfg, baseUrl: '', apiKey: '' });
  }, [props]);
        ...
        {props.configs.map((c) => (
          <view className="result-item" key={c.id}>
            ...
            <Input
              ...
              value={c.baseUrl ?? ''}
              bindinput={(e: BindEvt) => onBase(c, e.detail?.value ?? '')}
            />
            ...
            <Input
              ...
              placeholder="sk-..."
              value={c.apiKey ?? ''}
              bindinput={(e: BindEvt) => onKey(c, e.detail?.value ?? '')}
            />

            <view className="row" style={{ marginTop: 8 }}>
              <view className="btn" bindtap={() => toggle(c)}>
                <text>{c.enabled ? 'Disable' : 'Enable'}</text>
              </view>
              <view className="link small" style={{ marginLeft: 12 }} bindtap={() => clear(c)}>
                <text>Clear</text>
              </view>
            </view>
          </view>
