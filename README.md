...
Made up of multiple subagents located in [agents/softhints.py](agents/soft_hints.py), it first detects potential data such as location, etc. Then, it utilises google_search and url_context tool to search for related content to identify (if possible) data about the user (e.g location)
